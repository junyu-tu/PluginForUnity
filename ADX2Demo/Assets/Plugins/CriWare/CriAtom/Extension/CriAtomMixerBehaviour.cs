				...
				if (clipBehaviour != null) {
					if (m_Director.time >= clip.end ||
						...
						if (clipBehaviour.IsClipPlaying == false) { /* Entering clip for the first time */
							if (IsEditor == false) {
								clipBehaviour.Play(m_Bind, new CriAtomClipPlayConfig(criAtomClip.cueSheet, criAtomClip.cueName, seekTimeMs, clip.timeScale));
							} else {
								clipBehaviour.PreviewPlay(m_Guid, isDirectorPaused, new CriAtomClipPlayConfig(criAtomClip.cueSheet, criAtomClip.cueName, seekTimeMs, clip.timeScale));
	...
	public class CriAtomTimelinePreviewer : IDisposable {
		...
		public CriAtomExAcb GetAcb(string cueSheetName) {
			...
			if (lastAcfFile != this.atom.acfFile) {
				...
				CriAtomEx.RegisterAcf(null, Path.Combine(CriWare.streamingAssetsPath, atom.acfFile));
			...
			if (this.acbTable.ContainsKey(cueSheetName)) {
				...
				foreach (var cuesheet in atom.cueSheets) {
					if (cuesheet.name == cueSheetName) {
						string acbPath = (string.IsNullOrEmpty(cuesheet.acbFile)) ? null : Path.Combine(CriWare.streamingAssetsPath, cuesheet.acbFile);
						string awbPath = (string.IsNullOrEmpty(cuesheet.awbFile)) ? null : Path.Combine(CriWare.streamingAssetsPath, cuesheet.awbFile);
