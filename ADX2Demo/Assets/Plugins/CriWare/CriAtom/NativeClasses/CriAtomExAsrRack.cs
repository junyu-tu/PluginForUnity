...
public partial class CriAtomExAsrRack : CriDisposable
{
	...
	public CriAtomExAsrRack(Config config, PlatformConfig platformConfig)
	{
		...
		this._rackId = CRIWAREC9EB44D2(ref config, ref platformConfig);
	...
	private static extern int CRIWAREC9EB44D2([In] ref Config config, [In] ref PlatformConfig platformConfig);
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	...
	#else
	private static int CRIWAREC9EB44D2([In] ref Config config, [In] ref PlatformConfig platformConfig) { return 0; }
