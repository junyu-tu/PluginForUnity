...
public class CriAtomExAcb : CriDisposable
{
	 ...
	 * criAtomExAcb.Dispose 関数内で長時間処理がブロックされる可能性があります。）<br/>
	 */
	...
	/**
	 * <summary>復号キーの設定</summary>
	 * <param name="key">復号キー</param>
	 * <param name="nonce">ノンス(必ず 0 を指定してください)</param>
	 * \par 説明:
	 * ACBデータの暗号を解除するための復号キーを指定します。<br>
	 * ACBファイルをロードした後に本関数を実行することで、ACBハンドルごとに
	 * 異なる復号キーを使用することが可能です。<br>
	 * （デクリプタによる設定を無視して、当該ACBファイルについては本関数で指定
	 * した復号キーが使用されます。）
	 */
