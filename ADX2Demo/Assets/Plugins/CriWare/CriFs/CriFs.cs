...
public class CriFsLoader : CriDisposable
{
	...
	/**
	 * <summary>圧縮データを展開せずにメモリ上にロードします。</summary>
	 * <param name="binder">バインダ</param>
	 * <param name="path">ファイルパス名</param>
	 * <param name="fileOffset">ファイルの先頭からのオフセット位置（単位はバイト）</param>
	 * <param name="loadSize">ロード要求サイズ（単位はバイト）</param>
	 ...
	 * 指定されたバインダとファイル名で、データの読み込みを開始します。<br/>
	 * ファイル内のfileOffsetバイト目から、loadSizeバイト分読み込みます。<br/>
	 * ::CriFsLoader::Load 関数と異なり、データが圧縮されている場合でも、 データを展開せずにメモリ上にロードします。<br/>
	 * 本関数は即時復帰関数です。<br/>
	 * ロードの完了状態を取得するには ::CriFsLoader::GetStatus 関数を使用してください。<br/>
	 * \sa CriFsLoader::GetStatus, CriFsLoader::LoadWithoutDecompressionById, CriFsLoader::DecompressData
	 */
	 ...
	 * 本関数は即時復帰関数です。<br/>
	 * ロードの完了状態を取得するには ::CriFsLoader::GetStatus 関数を使用してください。<br/>
	 * \sa CriFsLoader::GetStatus, CriFsLoader::LoadWithoutDecompression, CriFsLoader::DecompressData
	 */
	...
	/**
	 * <summary>メモリ上に配置された圧縮データの展開を開始します。</summary>
	 * <param name="srcSize">圧縮データサイズ（単位はバイト）</param>
	 * <param name="srcBuffer">圧縮データが配置されたバッファ</param>
	 * <param name="dstSize">展開先バッファのサイズ（単位はバイト）</param>
	 * <param name="dstBuffer">展開先バッファ</param>
	 * \par 説明:
	 * メモリ上に配置された圧縮データを、別のメモリ領域に展開します。<br/>
	 * 本関数は即時復帰関数です。<br/>
	 * ロードの完了状態を取得するには criFsLoader_GetStatus関数を使用してください。<br/>
	 * \par 備考：
	 * 入力データが圧縮されていない場合、 本関数は入力データを出力アドレスへそのままコピーします。<br/>
	 * \attention
	 * 本関数は、CRI独自のソフトウェア圧縮コーデックにしか対応していません。<br/>
	 * ハードウェアデコーダを使用する場合や、プラットフォーム固有のコーデックを使用している場合、 
	 * 本関数ではデータを展開できません。
	 * \sa CriFsLoader::GetStatus, CriFsLoader::LoadWithoutDecompression, CriFsLoader::LoadWithoutDecompressionById
	 */
...
public class CriFsBinder : CriDisposable
{
	...
	/**
	 * <summary>コンテンツファイル情報構造体</summary>
	 * \par 説明:
	 * ::CriFsBinder::GetContentsFileInfo 関数の出力情報です。<br/>
	 * 検索したCPKファイルのコンテンツファイルにアクセスするための情報を格納します。<br/>
	 * \sa CriFsBinder::GetContentsFileInfo(int, out ContentsFileInfo), CriFsBinder::GetContentsFileInfo(string, out ContentsFileInfo)
	 */
	[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
	public struct ContentsFileInfo {
		[MarshalAs(UnmanagedType.LPStr)]
		public readonly string directory;	/**< ディレクトリ名								*/
		[MarshalAs(UnmanagedType.LPStr)]
		public readonly string fileName;	/**< ファイル名									*/
		public uint readSize;				/**< 読み込みサイズ（ファイルサイズ）			*/
		public uint extractSize;			/**< 展開サイズ（非圧縮の場合、readSizeと同値）	*/
		public ulong offset;				/**< CPKファイルの先頭からのオフセット位置		*/
		public int fileId;					/**< ファイルID	*/
		[MarshalAs(UnmanagedType.LPStr)]
		string userStr;						/**< ユーザ文字列（非対応）	*/
	...
	/**
	 * <summary>CPKコンテンツファイル情報を取得します。</summary>
	 * <param name="path">ファイルのフルパス</param>
	 * <param name="info">コンテンツファイル情報</param>
	 * <returns>取得に成功したか</returns>
	 * \par 説明：
	 * ID+ファイル名情報付きCPKファイルから、指定されたファイル名のファイル情報を取得します。<br/>
	 * 指定されたファイルを格納しているCPKが、ID+ファイル名情報付CPKである必要があります。<br/>
	 * 指定したバインダハンドルに、同じ名前のファイルが複数存在する場合、最初に見付けたファイルを
	 * 格納しているCPKが選択されます。
	 * \sa CriFsBinder::GetContentsFileInfo(int, out ContentsFileInfo)
	 */
	 ...
	 * 格納しているCPKが選択されます。
	 * \sa CriFsBinder::GetContentsFileInfo(string, out ContentsFileInfo)
	 */
	 ...
	 * <summary>CPKコンテンツファイル情報を取得します。</summary>
	 * <param name="bindId">バインドID</param>
	 * <param name="index">情報を取得するコンテンツファイルの先頭インデクス</param>
	 * <param name="numFiles">情報を取得するコンテンツファイル数</param>
	 * <param name="info">コンテンツファイル情報の配列</param>
	 * <returns>取得に成功したか</returns>
	 * \par 説明：
	 * ID+ファイル名情報付きCPKファイルから、指定されたインデクスから指定個数分ののファイル情報を取得します。<br/>
	 * インデクスはCPK作成時、コンテンツファイルに対して0から割り振られます。<br/>
	 * index 及び numFiles に指定できる最大値は、 ::CriFsBinder::GetNumContentsFiles で取得可能なファイル総数になります。
	 * \sa CriFsBinder::GetContentsFileInfo(string, out ContentsFileInfo), CriFsBinder::GetNumContentsFiles
	 */
	...
	/**
	 * <summary>CPKコンテンツファイルの総数を取得します。</summary>
	 * <param name="bindId">バインドID</param>
	 * <returns>コンテンツファイル数</returns>
	 * \par 説明：
	 * ID+ファイル名情報付きCPKファイルから、含まれるファイルの総数を取得します。<br/>
	 * 本関数の戻り値が、同じバインドIDに対して ::CriFsBinder::GetContentsFileInfoByIndex 関数で指定可能な
	 * 情報を取得するコンテンツファイル数の最大値になります。
	 * \sa CriFsBinder::GetContentsFileInfoByIndex
	 */
	public static int GetNumContentsFiles(uint bindId)
	{
		return CRIWARE5031326F(bindId);
	}
	...
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern int CRIWARE5031326F(uint id);
	...
	private static int CRIWARE5031326F(uint id) { return 0; }
	#endif
...
public static class CriFsUtility
{
	...
	{
		/* User-Agent文字列を指定 */
		CRIWARE860060D3(userAgentString);
	}
	...
	{
		/* プロキシ設定 */
		CRIWAREE0AE440A(proxyPath, proxyPort);
	}
	...
	{
		CRIWARE96AC7F8A(filter);
	}

	#region DLL Import
	#if !CRIWARE_ENABLE_HEADLESS_MODE
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern bool CRIWARE860060D3(string userAgentString);

	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern bool CRIWAREE0AE440A(string proxyPath, UInt16 proxyPort);

	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern bool CRIWARE96AC7F8A(string filter);
	#else
	private static bool CRIWARE860060D3(string userAgentString) { return true; }
	private static bool CRIWAREE0AE440A(string proxyPath, UInt16 proxyPort) { return true; }
	private static bool CRIWARE96AC7F8A(string filter) {return true; }
	#endif
	#endregion
}
...
public static class CriFsPlugin
{
	...
	public static void InitializeLibrary()
	{
		...
		/* ライブラリの初期化 */
		CriFsPlugin.CRIWARED7821B7B();
	}

	public static bool IsLibraryInitialized()
	{
		/* ライブラリが初期化済みかチェック */
		return CRIWARE186FD4E5();
	}
	...
	public static void FinalizeLibrary()
	{
		...
		/* ライブラリの終了 */
		CriFsPlugin.CRIWARE256A63C6();
	}

	#region DLL Import
	#if !CRIWARE_ENABLE_HEADLESS_MODE
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern void CRIWAREA939B536(
	...
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern void CRIWARED7821B7B();

	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	public static extern bool CRIWARE186FD4E5();

	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern void CRIWARE256A63C6();

	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	public static extern uint CRIWARE419EF258();
	...
	private static void CRIWAREA939B536(
		int num_loaders, int num_binders, int num_installers, int max_path, bool minimize_file_descriptor_usage, bool enable_crc_check) { }
	private static void CRIWARED7821B7B() { _dummyInitialized = true; }
	public static bool CRIWARE186FD4E5() { return _dummyInitialized; }
	private static void CRIWARE256A63C6() { _dummyInitialized = false; }
	public static uint CRIWARE419EF258() { return 0u; }
