...
public class CriAtomWindowInfo {
		...
		foreach (string file in files) {
			...
			if (acb != null) {
				...
				foreach (CriAtomEx.CueInfo cueInfo in cueInfoList) {
					...
					if (found == false) {
						acbInfo.cueInfoList.Add(new CueInfo(cueInfo.name, cueInfo.id, cueInfo.userData, Convert.ToBoolean(cueInfo.headerVisibility)));
					} else {
	...
	private string TryGetRelFilePath(string path) {
		Uri streamingAssetsUri = new Uri(CriWare.streamingAssetsPath);
